...
def read_input(screen, graph, pos):
    ...
    for n in path:
        screen.addch(
            n[1], n[0], PATH, curses.color_pair(GREEN) | curses.A_REVERSE)
    screen.refresh()
    ...
    screen.addch(
        last[1], last[0], PATH, curses.color_pair(BLUE) | curses.A_REVERSE)
    screen.addstr(
        1, 1, 'commands: {}, minutes: {}'.format(len(path) + 1, minutes))
    screen.getch()
    return HALT
